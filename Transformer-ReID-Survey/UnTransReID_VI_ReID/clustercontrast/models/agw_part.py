...
class embed_net_ori(nn.Module):
    def __init__(self,  num_classes=1000, no_local= 'on', gm_pool = 'on', arch='resnet50'):
        ...
        self.bottleneck_1 = nn.BatchNorm1d(pool_dim)
        self.bottleneck_1.bias.requires_grad_(False)  # no shift
        self.bottleneck_1.apply(weights_init_kaiming)

        self.bottleneck_2 = nn.BatchNorm1d(pool_dim)
        self.bottleneck_2.bias.requires_grad_(False)  # no shift
        self.bottleneck_2.apply(weights_init_kaiming)

        self.bottleneck_3 = nn.BatchNorm1d(pool_dim)
        self.bottleneck_3.bias.requires_grad_(False)  # no shift
        self.bottleneck_3.apply(weights_init_kaiming)

        self.bottleneck_4 = nn.BatchNorm1d(pool_dim)
        self.bottleneck_4.bias.requires_grad_(False)  # no shift
    ...
    def forward(self, x1, x2, modal=0,label_1=None,label_2=None,cid_rgb=None,cid_ir=None,index_rgb=None,index_ir=None):
        ...
        if self.gm_pool  == 'on':#######part
            b, c, h, w = x.shape
            x = x.view(b, c, -1)
            patch_length = x.size(2) // 4
            p = 3.0

            local_feat_1 = x[:,:, :patch_length]
            local_feat_1 = (torch.mean(local_feat_1**p, dim=-1) + 1e-12)**(1/p)


            # lf_2
            local_feat_2 = x[:,:, patch_length:patch_length*2]
            local_feat_2 = (torch.mean(local_feat_2**p, dim=-1) + 1e-12)**(1/p)


            # lf_3
            local_feat_3 = x[:,:, patch_length*2:patch_length*3]
            local_feat_3 = (torch.mean(local_feat_3**p, dim=-1) + 1e-12)**(1/p)


            # lf_4
            local_feat_4 = x[:,:, patch_length*3:patch_length*4]
            local_feat_4 = (torch.mean(local_feat_4**p, dim=-1) + 1e-12)**(1/p)
        ...
        feat = torch.cat((feat_g,local_feat_1_bn,local_feat_2_bn,local_feat_3_bn,local_feat_4_bn),dim=1)


        if self.training:
            return feat,feat[:single_size],feat[single_size:],label_1,label_2,cid_rgb,cid_ir,index_rgb,index_ir#,x_pool[:single_size],x_pool[single_size:]  #x_pool#, self.classifier(feat)
        else:
