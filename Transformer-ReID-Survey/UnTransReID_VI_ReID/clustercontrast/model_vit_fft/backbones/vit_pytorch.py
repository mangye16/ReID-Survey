...
class PatchEmbed_fft(nn.Module):
    """ Image to Patch Embedding with overlapping patches
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, embed_dim=768, stem_conv=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = torch.fft.fft(torch.fft.fft(x, dim=-1), dim=-2)  #torch.fft.fft(a)
        # print('before flatten',x.size())
        x = x.flatten(2).transpose(1, 2) # [18,9,768]  [64, 8, 768] 
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., camera=0, view=0,drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6), local_feature=False, sie_xishu =1.0, hw_ratio=1, gem_pool = False, stem_conv=True):
        ...
        self.patch_embed_fft = PatchEmbed_fft(
            img_size=img_size, patch_size=patch_size, stride_size=stride_size, in_chans=in_chans,
            embed_dim=embed_dim, stem_conv = stem_conv) 
    ...
    def forward(self, x1, x2, modal=0,label_1=None,label_2=None):#(self, x, camera_id, view_id):
        # B = x.shape[0]
        # x = self.patch_embed(x)

        single_size = x1.size(0)
        if modal == 0:
            x_fft =self.patch_embed_fft(torch.cat((x1, x2), 0))
            # print(x_fft)
            ...
            # x_fft = torch.fft.fft(torch.fft.fft(x, dim=-1), dim=-2).flatten(2).transpose(1, 2)
            # x = x.flatten(2).transpose(1, 2)

        elif modal == 1:
            x_fft =self.patch_embed_fft(x1)
            x = self.patch_embed(x1)
        elif modal == 2:
            x_fft =self.patch_embed_fft(x2)
