...
class TransReID(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=0, embed_dim=768,
                 depth=12, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, pretrained_path='',hw_ratio=1, conv_stem=False):
        super().__init__()
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))  # num_patches + num_tokens
    ...
    def forward_features(self, x, mask):
        ...
        if mask:
            ...
            x, mask_, ids_restore = self.random_masking(patches, 0.75)
    ...
    def forward(self, x, mask=False):
        ...
        if mask:
            x_mask = self.forward_features(x, mask)
            x_mask = F.normalize(x_mask[:, 0])
            return x_mask
        else:
            x = self.forward_features(x, mask)
            bn_x = feat = self.bottleneck(x[:, 0])
    ...
    def load_param(self, model_path, hw_ratio):
        ...
        for k, v in param_dict.items():
            ...
            if 'patch_embed.proj.weight' in k and len(v.shape) < 4:
                ...
                # v = resize_pos_embed_origin(v, self.pos_embed, self.patch_embed.num_y, self.patch_embed.num_x)
            try:
...
def resize_pos_embed_origin(posemb, posemb_new, hight, width):
...
def resize_pos_embed_wacv(posemb, posemb_new, hight, width, hw_ratio):
    # Rescale the grid of position embeddings when loading from state_dict. Adapted from
    # https://github.com/google-research/vision_transformer/blob/00883dd691c63a6830751563748663526e811cee/vit_jax/checkpoint.py#L224
    ntok_new = posemb_new.shape[1]
