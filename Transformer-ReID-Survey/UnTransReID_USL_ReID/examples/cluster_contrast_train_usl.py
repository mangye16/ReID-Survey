...
def main_worker(args):
    ...
    for epoch in range(args.epochs):
        with torch.no_grad():
            ...
            features, _ = extract_features(model, cluster_loader, epoch, print_freq=50)
        ...
        cluster_center = F.normalize(cluster_features, dim=1).cuda()
        memory.features = cluster_center
        trainer.memory = memory
        trainer.cluster_center = cluster_center
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pp', '--pretrained-path', type=str,
                        default='vit_base_ics_cfs_lup.pth')
    ...
    parser.add_argument('--temp', type=float, default=0.05,  # 0.05
